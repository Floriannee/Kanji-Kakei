    ...
    def parse_receipt_image(self, pil_image: Image.Image, allow_simulation: bool = True, max_retries: int = 5) -> dict:
        ...
        if response and "items" in response:
            for item in response["items"]:
                ...
                if any(word in jp_name for word in ["ファミチキ", "ナナチキ", "エルチキ", "からあげクン"]) or \
                   any(word in eng_name for word in ["famichiki", "nanachiki", "l-chiki", "karaage-kun", "fried chicken"]):
                    item["category"] = "Snack"
                elif "rice ball" in eng_name or "onigiri" in eng_name or "hand roll" in eng_name or "bread" in eng_name or "pan" in eng_name or "pastry" in eng_name or "croissant" in eng_name or "donut" in eng_name or \
